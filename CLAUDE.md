...
- **Redis**（默认 `localhost:6379/db=0`）：LangGraph checkpointer，保存多轮会话历史。连不上会降级为无状态 agent。
- **Milvus**（默认 `http://localhost:19530`）：向量库。不可达时 `/v1/rag/*` 接口返回 503；但 agent 启动只 import 工具函数、不立即连接，单 RAG 故障不会拖垮整个应用。

LLM 走 DeepSeek（`OPENAI_API_BASE=https://api.deepseek.com`），embedding 用阿里 DashScope `text-embedding-v4`（2048 维）。
...
| 路径 | 职责 |
|---|---|
| `app/main.py` | FastAPI 入口；`load_dotenv(".env")` 最先执行；注册中间件、异常处理器、`chat_router`、`rag_router`。保留 `/items/*` demo 路由供 `test_main.py` 覆盖。 |
| `app/api/v1/chat.py` | OpenAI 兼容 `/v1/chat/completions`（流式 + 非流式）与 `/v1/models`。 |
| `app/api/v1/rag.py` | RAG REST：`/v1/rag/documents` 插入/列表/删除、`/v1/rag/search` 父子语义检索、`/v1/rag/search/fulltext` BM25 字面检索、`/v1/rag/collections` 管理。每次调用前 `_ensure_ready()` 确保 Milvus 集合存在。 |
| `app/services/chat_service.py` | OpenAI ↔ LangChain 消息映射 + 流式/非流式分流（见上文）。 |
| `app/agent/` | LangChain Agent 工厂（`main.py`）、DeepSeek LLM 实例（`llm/`）、Redis checkpointer 工厂（`config/redis_config.py`）、`CustomAgentState`（`state/`）、孤儿 tool_calls 清洗中间件（`middleware/context.py`）。 |
| `app/mcp/` | MCP 服务端（`main.py`，stdio 启动）+ `client.py` 提供 `tools_context()` 异步上下文管理器，加载工具时挂 `_CallLogger` 拦截器在 stdout 打印每次 tool 调用与结果。 |
| `app/rag/` | `embedding.py`（Qwen text-embedding-v4 单例）、`document_rag/`（service/repository/schemas/chunking/language/config 分层，见下）、`raw_documents.json` 测试数据。 |
| `app/core/` | 跨切面：`lifespan.py`、`logging.py`（按大小切割 + 请求 ID 注入）、`middleware.py`（`X-Request-ID` + contextvars）、`errors.py`（按路径前缀分流错误格式）、`context.py`（contextvars）。 |
| `app/schemas/` | Pydantic 模型：`openai_types.py`（OpenAI 兼容请求/响应）、`rag_types.py`（RAG DTO）。 |
| `app/routers/` `app/dependencies/` `app/static/` `app/tests/` | FastAPI 教程样板，仅 `tests/test_main.py` 仍被 `pytest` 收集（`X-Token: coneofsilence` 硬编码）。`dependencies.py` 的 `get_token_header` / `get_query_token` 未被 `main.py` 引用。 |

### RAG 子包（`app/rag/document_rag/`）

两级 Milvus 集合：`chapter_collection`（父，整章节文本 + BM25 sparse_vector）与 `sentence_collection`（子，sentence chunk dense vector）。检索默认走**父子查询**：先在 sentence 层召回 top-N chunk → 聚合到 doc/chapter → 返回 top docs；另有 BM25 全文检索路径用于精确术语/型号字面匹配。
...
- **pytest 绝对导入**：`pythonpath=["."]` 已配，测试用 `from app.main import app`，不要改成相对导入。
